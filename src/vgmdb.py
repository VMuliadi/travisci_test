...
def get_vgmdb(vgmdb_url):
    album_information = {}
    vgmdb_request = requests.get(vgmdb_source)
    ...
    try:
        album_information['release_year'] = general_info.findAll('tr')[1].findAll('td')[1].get_text().split(',')[1].split()[0]
        ...
        album_information['release_year'] = general_info.findAll('tr')[4].findAll('td')[1].get_text().split(',')[1].split()[0]
